...
#define NRF_CLOCK_LFCLKSRC                                                                                                                 \
  {                                                                                                                                        \
    .source = NRF_CLOCK_LF_SRC_XTAL, .rc_ctiv = 0, .rc_temp_ctiv = 0, .xtal_accuracy = NRF_CLOCK_LF_XTAL_ACCURACY_30_PPM                   \
  }

#define IS_SRVC_CHANGED_CHARACT_PRESENT                                                                                                    \
  0 /**< Include the service_changed characteristic. If not enabled, the server's database cannot be changed for the lifetime of the       \
       device. */
...
#define MIN_CONN_INTERVAL                                                                                                                  \
  MSEC_TO_UNITS(20, UNIT_1_25_MS) /**< Minimum acceptable connection interval (20 ms), Connection interval uses 1.25 ms units. */
#define MAX_CONN_INTERVAL                                                                                                                  \
  MSEC_TO_UNITS(75, UNIT_1_25_MS) /**< Maximum acceptable connection interval (75 ms), Connection interval uses 1.25 ms units. */
#define SLAVE_LATENCY 0           /**< Slave latency. */
#define CONN_SUP_TIMEOUT                                                                                                                   \
  MSEC_TO_UNITS(4000, UNIT_10_MS) /**< Connection supervisory timeout (4 seconds), Supervision Timeout uses 10 ms units. */
#define FIRST_CONN_PARAMS_UPDATE_DELAY                                                                                                     \
  APP_TIMER_TICKS(5000, APP_TIMER_PRESCALER) /**< Time from initiating event (connect or start of notification) to first time              \
                                                sd_ble_gap_conn_param_update is called (5 seconds). */
#define NEXT_CONN_PARAMS_UPDATE_DELAY                                                                                                      \
  APP_TIMER_TICKS(30000,                                                                                                                   \
                  APP_TIMER_PRESCALER) /**< Time between each call to sd_ble_gap_conn_param_update after the first call (30 seconds). */
...
void app_error_fault_handler(uint32_t id, uint32_t pc, uint32_t info)
{
  ...
  if (error_info != NULL) {
    uint8_t buf[256];
    snprintf((char *)buf, 256, "Error file:%s, line:%d, code:%d", error_info->p_file_name, error_info->line_num, (int)error_info->err_code);
    NRF_LOG_ERROR("%s\r\n", (int32_t)buf);
  } else {
    NRF_LOG_ERROR("Fatal\r\n");
  }
